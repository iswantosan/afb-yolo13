        ...
        if m in {
            ...
            if m in {C3k2, DSC3k2}:  # for P/U sizes
                ...
                if scale in "lx":
                    args[3] = True
            if m is RodDSC3k2:
                # Args after insert: [c1, c2, n, shortcut, e, g, k]
                # No dsc3k flag (single internal block). Keep as-is.
                legacy = False
            if m is A2C2f:
                legacy = False
            ...
            c2 = ch[f[-1]]
        elif m in {HyperACE, HyperACEScale}:  # AFB: HyperACEScale shares HyperACE args
            legacy = False
            c1 = ch[f[1]]
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            ...
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        elif m is IBNConv:  # AFB: hybrid IN+BN drop-in for Conv (stem style invariance)
            c1 = ch[f]
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
        elif m is WTConv:  # AFB: wavelet-transform conv (background/foreground freq separation)
            c1 = ch[f]
            c2 = args[0]
            c2 = make_divisible(min(c2, max_channels) * width, 8)
            args = [c1, c2, *args[1:]]
