...
def nwd_distance(pred_xyxy, target_xyxy, C: float = 12.8):
    ...
    Implements NWD from Wang et al. (2022) "A Normalized Gaussian Wasserstein
    Distance for Tiny Object Detection" (arXiv:2110.13389).

    Each bbox is treated as a 2D Gaussian centered at the box center with
    diagonal covariance proportional to box size. The Wasserstein-2 distance
    between such Gaussians admits a simple closed form. NWD = exp(-sqrt(W2)/C).

    Particularly suited to:
      - small / tiny objects where IoU is unstable (zero gradient when boxes
        don't overlap)
      - elongated / rod-shape objects where CIoU's aspect ratio penalty
        misbehaves on extreme aspects (typical for ZN-stained AFB bacilli)

    Args:
        pred_xyxy:   (N, 4) predicted boxes in xyxy.
        target_xyxy: (N, 4) target boxes in xyxy.
        C:           Normalization constant. Default 12.8 follows the original
                     paper for small obj. Larger C -> harsher distance scaling.

    Returns:
        (N,) tensor of NWD similarity in (0, 1].
    """
    ...
    # Squared L2 between centers + squared diff of half-sizes
    center_dist_sq = (pred_cx - tgt_cx).pow(2) + (pred_cy - tgt_cy).pow(2)
    wh_dist_sq = ((pred_w - tgt_w) * 0.5).pow(2) + ((pred_h - tgt_h) * 0.5).pow(2)
    w2 = center_dist_sq + wh_dist_sq  # Wasserstein-2 distance squared

    return torch.exp(-w2.sqrt() / C)


class BboxLoss(nn.Module):
    """Criterion class for computing training losses during training.

    Supports optional hybrid CIoU + NWD (Normalized Wasserstein Distance) loss:
      loss = (1 - nwd_ratio) * (1 - CIoU) + nwd_ratio * (1 - NWD)

    NWD targets HIGH `LOSS_LOCALIZATION` flag for small / elongated objects
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # Hybrid NWD term (Wang et al. 2022, arXiv:2110.13389).
        if self.nwd_ratio > 0:
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        # AFB-YOLOv13: optional NWD term in BboxLoss (Wang et al. 2022).
