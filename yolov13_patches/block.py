...
# ==============================================================================
# AFB-YOLOv13 custom modules (added for Tuberculosis6208 / bacilli detection)
# ==============================================================================

class RodDSBottleneck(nn.Module):
    """Asymmetric rod-shape depthwise bottleneck for elongated biological objects.

    Replaces standard DSBottleneck's sequential isotropic depthwise convs with
    three parallel depthwise branches:
      - dw_h: (1 x k) horizontal rod kernel
      - dw_v: (k x 1) vertical rod kernel
      - dw_s: (3 x 3) isotropic baseline
    Branches are summed then projected via 1x1 pointwise conv.

    Motivation:
        Bacilli (e.g. AFB / TB) in ZN-stained microscopy appear as elongated rods
        (~1-4 um x 0.2-0.5 um, ~4-8:1 aspect ratio) at arbitrary orientations.
        Symmetric depthwise kernels (3x3, 5x5, 7x7) waste capacity smoothing
        non-aspect-matching directions. Parallel asymmetric rod kernels capture
        horizontal + vertical orientations efficiently; the central 3x3 retains
        general features and orientation-mixed responses.

    Args:
        c1 (int): Input channels.
        c2 (int): Output channels.
        shortcut (bool): Add residual when c1 == c2.
        e (float): Hidden expansion ratio.
        k (int): Rod kernel length (must be odd).
    """

    def __init__(self, c1, c2, shortcut=True, e=0.5, k=5):
        super().__init__()
        assert k % 2 == 1, "Rod kernel length must be odd"
        c_ = int(c2 * e)
        pad = k // 2
        # 1x1 entry to compress c1 -> c_
        self.entry_pw = nn.Conv2d(c1, c_, 1, bias=False)
        self.entry_bn = nn.BatchNorm2d(c_)
        # Three parallel depthwise branches
        self.dw_h = nn.Conv2d(c_, c_, (1, k), padding=(0, pad), groups=c_, bias=False)
        self.dw_v = nn.Conv2d(c_, c_, (k, 1), padding=(pad, 0), groups=c_, bias=False)
        self.dw_s = nn.Conv2d(c_, c_, 3, padding=1, groups=c_, bias=False)
        self.mid_bn = nn.BatchNorm2d(c_)
        self.act = nn.SiLU()
        # 1x1 projection back to c2
        self.proj_pw = nn.Conv2d(c_, c2, 1, bias=False)
        self.proj_bn = nn.BatchNorm2d(c2)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        y = self.act(self.entry_bn(self.entry_pw(x)))
        y = self.dw_h(y) + self.dw_v(y) + self.dw_s(y)
        y = self.act(self.mid_bn(y))
        y = self.proj_bn(self.proj_pw(y))
        return x + y if self.add else y


class RodDSC3k2(C2f):
    """CSP-style block using RodDSBottleneck for rod-shape feature extraction.

    Drop-in replacement for DSC3k2 in YAML. Same arg pattern (c2, ?, e, g,
    shortcut, k) except `dsc3k` arg is dropped (RodDSBottleneck is the only
    internal block).

    YAML usage: [-1, n, RodDSC3k2, [c2, shortcut, e, g, k]]
    """

    def __init__(self, c1, c2, n=1, shortcut=True, e=0.5, g=1, k=5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(
            RodDSBottleneck(self.c, self.c, shortcut=shortcut, e=1.0, k=k)
            for _ in range(n)
        )


class SpatialFullPAD_Tunnel(nn.Module):
    """Content-aware spatial-gated FullPAD tunnel.

    Replaces FullPAD_Tunnel's global scalar gate with a per-location gate
    predicted from the concatenation of the original and enhanced features.
    Output: x[0] + sigmoid(predicted_gate) * x[1], where predicted_gate is a
    single-channel spatial map [B, 1, H, W].

    Motivation:
        Baseline FullPAD_Tunnel applies HyperACE enhancement uniformly across
        all spatial locations (single learnable scalar). For sparse-positive
        scenes (microscopy: most pixels are background, bacilli appear in
        clusters), uniform enhancement may average out useful local signal.
        Spatial gate allows the network to enhance specific regions only.

    Args:
        channels (int): Number of channels per input tensor (both inputs same).
    """
    ...
    def forward(self, x):
        # x = list of [original, enhanced], same shape [B, C, H, W]
        gate = torch.sigmoid(self.gate_pred(torch.cat(x, dim=1)))
        return x[0] + gate * x[1]


class ScaleAwareFuseModule(nn.Module):
    """FuseModule + learnable per-scale embedding injected before fusion.

    Adds a learnable channel-wise bias to each of the three pre-fusion scale
    branches (P3-downsampled, P4, P5-upsampled). The bias acts as a scale-ID
    signal that downstream modules (HyperACE branches, AdaHyperedgeGen) can
    learn to leverage for scale-conditioned processing.

    Args:
        c_in (int): Channels per input feature map.
        channel_adjust (bool): Match FuseModule's channel_adjust behaviour.
    """
    ...
    def __init__(self, c_in, channel_adjust):
        ...
        self.downsample = nn.AvgPool2d(kernel_size=2)
        self.upsample = nn.Upsample(scale_factor=2, mode="nearest")
        # Learnable per-scale channel bias [3, c_in, 1, 1]
        self.scale_embed = nn.Parameter(torch.zeros(3, c_in, 1, 1))
    ...
    def forward(self, x):
        x1_ds = self.downsample(x[0]) + self.scale_embed[0]
        x2 = x[1] + self.scale_embed[1]
        x3_up = self.upsample(x[2]) + self.scale_embed[2]
        x_cat = torch.cat([x1_ds, x2, x3_up], dim=1)
        return self.conv_out(x_cat)


class HyperACEScale(nn.Module):
    """Scale-aware variant of HyperACE.

    Identical to HyperACE except FuseModule is replaced by ScaleAwareFuseModule
    which injects learnable per-scale (P3/P4/P5) channel embeddings before
    fusion. This provides a scale-ID prior that propagates into the C3AH
    branches' AdaHyperedgeGen, enabling implicit scale-conditioned hyperedge
    prototype specialization without requiring explicit per-vertex scale IDs.

    All other args/behaviour identical to HyperACE.
    """
    ...
    def __init__(self, c1, c2, n=1, num_hyperedges=8, dsc3k=True, shortcut=False,
        ...
        # The key change: scale-aware fusion
        self.fuse = ScaleAwareFuseModule(c1, channel_adjust)
        self.branch1 = C3AH(self.c, self.c, e2, num_hyperedges, context)
        self.branch2 = C3AH(self.c, self.c, e2, num_hyperedges, context)
...
class CoordAtt(nn.Module):
    ...
    Decomposes channel attention into two parallel 1D feature encodings along
    H and W. Preserves positional information along the orthogonal axis —
    well-suited for orientation-aware feature extraction on elongated targets
    (rod-shaped bacilli at arbitrary in-plane rotation).
    """
